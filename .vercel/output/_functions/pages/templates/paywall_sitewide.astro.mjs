import { g as getLocale, a as getContext, b as getEnv, s as shouldRenderCampaign, c as campaignVariant, h as htmlPage, d as detectCurrencyCode, e as escapeHtml, m as money, f as sanitizeImageUrl, i as sanitizeColor, j as sanitizePosition, k as sanitizeSize } from '../../chunks/paywall_ByJgr8mX.mjs';
...
async function GET({ request }) {
  ...
  const payload = await campaignResponse.json().catch(() => null);
  ...
  const body = variant === "monthly" ? renderMonthly(campaign, { locale, context, canSkip, showSurvey, dismissable, env }) : renderAnnual(campaign, { locale, context, canSkip, showSurvey, dismissable, env });
  return new Response(htmlPage(locale, body, "Sitewide Plan Dialog"), {
    headers: { "content-type": "text/html; charset=utf-8" }
  });
}
function renderAnnual(campaign, options) {
  ...
  return `<div class="layout-shell layout-shell--centered">
    <article class="modal-card sitewide-card">
      ...
      <div class="card-body">
        ...
        ${canSkip ? '<button class="btn btn-ghost js-close-dialog">Continue without switching</button>' : '<p class="text-sm">You can continue with your current access and switch later.</p>'}
      </div>
      ${sharedScripts()}
...
function renderMonthly(campaign, options) {
  const { locale, context, canSkip, showSurvey, dismissable, env } = options;
  const ctx = contextDetails(context, env);
  const pricing0 = campaign.pricingData?.[0] ?? {};
  const pricing1 = campaign.pricingData?.[1] ?? {};
...
function contextDetails(context, env) {
  const data = {
  ...
  return data[context ?? "courses"] ?? data.courses;
}
function mediaAndTimer(campaign) {
  ...
  const canShowTimer = timer?.days ? timeRemaining > 0 && timeRemaining < timer.days * 24 * 60 * 60 : false;
  const deadlineEpoch = Date.now() + timeRemaining * 1e3;
  ...
  return `<div class="media-block">
    <div class="media-panel" style="background-image: ${bgUrl ? `url('${escapeHtml(bgUrl)}')` : "none"}; background-color: ${escapeHtml(bgColor)}; background-position: ${escapeHtml(bgPos)}; background-size: ${escapeHtml(bgSize)};">
      ...
      ${canShowTimer ? `<div class="countdown-wrap">
...
function planHref(baseUrl, promoKey, utmSource, extra, period) {
